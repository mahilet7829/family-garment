...
class ProductionService {
  ...
  /// Returns the created ProductionLogModel.
  Future<ProductionLogModel> recordProduction({
    ...
    required Map<int, double> materialsToDeduct, // materialId -> quantity
    required Map<int, String> materialNames, // materialId -> name
  }) async {
    final db = await _db.database;

    // Use a transaction: all deductions succeed or all fail
    return await db.transaction((txn) async {
      // Step 1: Check all materials have sufficient stock
      for (var entry in materialsToDeduct.entries) {
        ...
        if (result.isEmpty) {
          throw Exception('Material ID $materialId not found');
        }

        final currentStock = (result.first['currentStock'] as num).toDouble();
        if (currentStock < deductQty) {
          throw Exception(
              'Insufficient stock for ${materialNames[materialId]}. '
              'Need: ${deductQty.toStringAsFixed(2)}, Have: ${currentStock.toStringAsFixed(2)}');
        }
      }

      // Step 2: Deduct all materials
      for (var entry in materialsToDeduct.entries) {
        final materialId = entry.key;
        final deductQty = entry.value;
      ...
      // Step 3: Build materials used summary
  ...
  /// Get all production logs, optionally filtered by date range.
  Future<List<ProductionLogModel>> getLogs({
    ...
    final maps = await db.query('production_logs',
        where: where,
        whereArgs: whereArgs,
...
// Extension for ProductionLogModel to add copyWith
