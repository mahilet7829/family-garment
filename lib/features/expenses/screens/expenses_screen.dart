...
class _ExpensesScreenState extends State<ExpensesScreen> {
  ...
  Future<void> _openDetail(Map<String, dynamic> expense) async {
    ...
    _load(); // Refresh when returning from detail
  }
        ...
        content: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            DropdownButtonFormField<String>(
              value: category,
              decoration: InputDecoration(labelText: 'Type', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
            ...
            DropdownButtonFormField<String>(
              value: frequency,
              decoration: InputDecoration(labelText: 'Frequency', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
  ...
  Future<void> _deleteExpense(Map<String, dynamic> expense) async {
    final c = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Text('Delete "${expense['name']}"? This will also delete all payment history.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
...
class _ExpenseDetailScreenState extends State<ExpenseDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final category = e['category'] as String;
    final frequency = (e['expenseFrequency'] as String?) ?? 'monthly';
    final totalPaid = _payments.fold(0.0, (sum, p) => sum + (p['amount'] as num).toDouble());

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(title: Text(e['name'] as String), backgroundColor: AppColors.navy, foregroundColor: AppColors.white, elevation: 0),
        ...
        Card(elevation: 1, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_payments.isEmpty) const Text('No payments recorded yet', style: TextStyle(color: AppColors.textSecondary))
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(CurrencyFormatter.format((p['amount'] as num).toDouble()), style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
